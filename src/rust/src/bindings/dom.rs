...
pub fn append_child(parent: &ExternRef, child: &ExternRef) {
    ...
    crate::js::invoke_and_return_number(code, &[InvokeParam::ExternRef(parent), InvokeParam::ExternRef(child)]);
}

pub fn alert(message: &str) {
    let code = "function(message){ alert(message); }";
    crate::js::invoke_and_return_number(code, &[InvokeParam::String(message)]);
}
...
pub fn element_set_inner_html(element: &ExternRef, html: &str) {
    ...
    crate::js::invoke_and_return_number(code, &[InvokeParam::ExternRef(element), InvokeParam::String(html)]);
}

pub fn element_add_class(element: &ExternRef, class: &str) {
    let code = "function(element, c){ element.classList.add(c); }";
    crate::js::invoke_and_return_number(code, &[InvokeParam::ExternRef(element), InvokeParam::String(class)]);
}
...
pub fn element_set_style_attribute(element: &ExternRef, attribute: &str, value: &str) {
    ...
    crate::js::invoke_and_return_number(code, &[InvokeParam::ExternRef(element), InvokeParam::String(attribute), InvokeParam::String(value)]);
}
...
pub fn element_remove(element: &ExternRef) {
    ...
    crate::js::invoke_and_return_number(code, &[InvokeParam::ExternRef(element)]);
}
    ...
    let code = r#"
        function(element ){
            const handler = (e) => {
                ...
                const buffer = (new TextEncoder()).encode(text);
                const allocationId = writeBufferToMemory(buffer);
                wasmModule.instance.exports.web_handle_change_event(BigInt(objectId), allocationId);
            };
...
pub fn element_add_click_listener(element: &ExternRef, handler: impl FnMut(MouseEvent) + 'static) -> Rc<ExternRef> {
    ...
    let code = r#"
        function(element ){
            const handler = (e) => {
                wasmModule.instance.exports.web_handle_mouse_event_handler(BigInt(objectId),e.offsetX, e.offsetY);
            };
            const objectId = storeObject(handler);
...
pub fn element_add_key_down_listener(element: &ExternRef, handler: impl FnMut(KeyboardEvent) + 'static) -> Rc<ExternRef> {
    let code = r#"
        function(element ){
            const handler = (e) => {
                wasmModule.instance.exports.web_handle_keyboard_event_handler(BigInt(objectId),e.keyCode);
            };
            const objectId = storeObject(handler);
